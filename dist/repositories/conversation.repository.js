...
class ConversationRepository {
    ...
    async createConversation(data) {
        await conversation_1.ConversationModel.create({
            type: data.type,
    ...
    async addUsersInGroupUsingConversationId(data) {
        await conversation_1.ConversationModel.updateOne({ _id: data.conversation_id }, { $addToSet: { participants: { $each: data.user_id } } });
    }
    //
    async getAllConversationForMyUserId(my_user_id) {
        const userId = new mongoose_1.Types.ObjectId(my_user_id);
